...
void UFlowAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property && (PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UFlowAsset, CustomInputs)
		...
		|| PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UFlowAsset, Properties)))
	{
...
void UFlowAsset::TriggerInput(const FConnectedPin& InConnectedPin, const EFlowPinActivationType ActivationType /*= Default*/)
{
	if (UFlowNode* Node = Nodes.FindRef(InConnectedPin.NodeGuid))
	{
		...
		Node->TriggerInput(InConnectedPin.PinName, ActivationType);
	}
}
